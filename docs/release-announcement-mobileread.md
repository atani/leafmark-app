...
- **AI 生成っぽい文体は攻撃対象になる**。2026-06 の ScrollWizard スレッド
  （showthread.php?t=374142）では「投稿文と App Store 説明文が AI 生成っぽい =
  vibe-coded アプリだろう」と断定され門前払いされた。機能の箇条書きを並べた
  ブローシャー調は避け、一人称の短い開発ストーリーで書く
- 返信も必ず一人称（I）で書く。会社口調・三人称の返信は同スレッドで逆効果だった
- 宣伝色を抑える。価格の数字は本文に書かない
- スクリーンショット添付: ライブラリ 3 冊 / iPad 2 カラム / ハイライト + ノート / 統計画面
- Thread Subscription を Instant email notification にして返信を見逃さない
- 返信での機能要望はすべて docs/TODO.md に転記し、「無いと移行できない機能」に
  タグを付ける（GTM 戦略 6. 指標）
- 競合スレッドの観察メモ: ScrollWizard は Markdown エクスポート・OPDS・iCloud 同期
  まで無料で提供（$4.99 上位版は速読・分割リーダーのみ）。機能表で殴り合わず、
  「読み心地 × データの所有権」の軸から降りない（go-to-market の MarginNote 方針と同じ）。
  住人が挙げた既存の定番は Yomu。JSWolf（古参）はフォント weight 調整と
  フォント持ち込みを要求するのが定番質問と知っておく
