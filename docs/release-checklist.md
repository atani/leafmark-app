...
英語ASOメタデータを更新するためのリリース。アプリ機能は v1.14 と同一。

- [x] バージョン番号・ビルド番号を 1.15 / 24 に設定
- [x] en-US のサブタイトルとキーワードを App Store Connect に保存
- [x] en-US の「このバージョンの最新情報」を草案化
- [x] ユニットテスト 164 件を実行。161 件成功、StoreKit 購入 3 件は既定どおりスキップ
- [x] Release アーカイブを作成し、build 24 を App Store Connect へアップロード
      — 2026-09-01 08:06 JST。Xcode のアップロードログで成功を確認
- [x] build 24 を選択し、承認後のリリースを手動に設定
- [x] App Reviewへ提出し、ステータス「審査待ち」を確認
      — 2026-09-01 08:22 JST。既存の ja ローカリゼーションでも What's New が
      必須だったため、英語のみの方針に合わせて en-US と同じ英文を設定
- [x] 審査を通過し、App Store で配信を開始
      — 2026-09-02 01:14 JST（2026-09-01 16:14 UTC）。App Store Connect の
      アクティビティ履歴と iTunes Lookup API の `currentVersionReleaseDate` が一致した
- [x] 審査の所要時間を記録（すべて JST。App Store Connect のアクティビティ履歴）
      ...
      | 09-01 08:22 | 審査待ち |
      | 09-01 23:34 | 審査中 |
      | 09-02 00:02 | デベロッパによるリリース待ち（審査通過） |
      | 09-02 01:14 | 配信準備完了 |

      提出から審査開始まで 15 時間 12 分、審査そのものは 28 分。
      リリースを手動に設定していたため、承認から配信までの 1 時間 12 分は
      こちらがボタンを押すまでの待ち時間である。次回、配信を急ぐ場合は
      承認直後に押せるよう、提出は日中に寄せる
- [x] 配信後の App 名を確認。en-US は `Leafmark: EPUB Reader & Notes`、
      ja は `Leafmark: EPUB リーダー & ノート` で反映されている
...
- [x] 冊数のカウントは「開いた」ではなく「読んだ」で数える。3 分以上開いていた
      本だけを数え、開いてすぐ閉じた本は数えない。閉じたときに出す作りへ変えた
      結果、build 22 では開いてすぐ戻るだけで評価を求める形になっていた
...
- [x] バージョン番号・ビルド番号を 1.14 / 23 に設定
- [x] en-US / ja の「このバージョンの最新情報」を設定
- [x] Release アーカイブを作成し、build 23 を App Store Connect へアップロード
- [x] build 21 の審査提出（2026-08-17 11:38 JST）を取り下げた。
      UX 確認で割り込みの問題が見つかったため。審査は始まっていない
- [x] build 23 を審査提出 — 2026-08-17 15:05 JST、ステータス「審査待ち」。
      CLI からの書き出しには配布用プロビジョニングプロファイルが要る。
      手元に 1 つも無かったため、App Store Connect API で
