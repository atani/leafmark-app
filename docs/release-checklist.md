...
- [ ] Release アーカイブを作成し、build 23 を App Store Connect へアップロード
- [ ] build 21 の審査提出（2026-08-17 11:38 JST）を取り下げる。
      UX 確認で割り込みの問題が見つかったため
- [ ] build 23 を審査提出する。CLI からの書き出しには配布用プロビジョニング
      プロファイルが要る。手元に 1 つも無かったため、App Store Connect API で
      `Leafmark App Store CLI` を作って手動署名で書き出している
